...
def main():
    input_path = "analysed_B5.csv"
    if not os.path.exists(input_path):
        print(f"Error: Calculated data '{input_path}' not found. Please run analytical_model.py first.")
        return
    ...
    print("Generating heat_gen vs cycle plot...")
    graph_dir_heat = "./graph/heat_gen vs cycle"
    ...
    plt.title("Heat Generation vs. Cycle (Battery B5, Charge Phase)", fontsize=14, fontweight="bold", pad=15)
    ...
    print("Generating avg_heat_gen vs cycle plot...")
    plt.figure(figsize=(10, 6))
    sns.lineplot(
        data=df, 
        x="cycle", 
    ...
    plt.title("7-Point Moving Average Heat Generation vs. Cycle (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    plt.xlabel("Cycle Number", fontsize=12)
    plt.ylabel("Heat Generation Rate (W)", fontsize=12)
    plt.legend(fontsize=11)
    plt.tight_layout()
    ...
    print("Generating temperature profile plot for Cycle 1...")
    ...
    temp_graph_dir = "./graph/temperature profile"
    ...
    plt.title("Radial Temperature Profile T(r) at Cycle 1 (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    ...
    print("Generating deltaT vs cycle plot...")
    delta_graph_dir = "./graph/deltaT vs cycle"
    ...
    plt.title("Core-to-Surface Temperature Difference (Delta T) vs. Cycle (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    plt.xlabel("Cycle Number", fontsize=12)
    ...
    plt.title("Radial Tangential Thermal Stress Profile σ_θ(r) at Cycle 1 (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    plt.xlabel("Radial Position r (mm)", fontsize=12)
    ...
    print("Generating stress vs cycle plot...")
    stress_cycle_dir = "./graph/stress vs cycle"
    ...
    plt.title("Peak Surface Tangential Thermal Stress vs. Cycle (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    plt.xlabel("Cycle Number", fontsize=12)
    ...
    print("Generating stress vs deltaT plot...")
    stress_deltaT_dir = "./graph/stress vs deltaT"
    ...
    plt.title("Thermal Stress vs. Core-to-Surface Temperature Difference (Delta T)", fontsize=14, fontweight="bold", pad=15)
    ...
    print("Generating cumulative_stress and SOH vs cycle plot...")
    cum_stress_dir = "./graph/cumulative_stress vs cycle"
    ...
    plt.title("SOH (%) and Cumulative Thermal Stress vs. Cycle (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    fig.tight_layout()
    ...
    print("Generating cumulative_stress vs SOH plot...")
    cum_stress_soh_dir = "./graph/cumulative_stress vs SOH"
    ...
    plt.title("State of Health (SOH %) vs. Cumulative Thermal Stress (Battery B5)", fontsize=14, fontweight="bold", pad=15)
    ...
    print("\nAll graphs successfully created and updated in their respective ./graph folders.")
